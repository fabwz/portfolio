...
- **Active section indicator:** as the user scrolls, the nav link corresponding to the currently-visible section is marked with `--accent` text color and a thin animated underline beneath it (fades/slides in, ~280ms transition — not an instant snap; slower than the initial 150-200ms, which felt too fast compared to every other transition in the project). Tracked via the same `IntersectionObserver` pattern already used for scroll-reveal, but continuous (tracks which section is most in-view at all times), not a one-time reveal.
...
- **Section heading alignment:** every section's `<h2>` (About, Projects, Skills, Contact) is horizontally centered within the container. This applies **only to the heading itself**, not necessarily to the content below it — About's paragraphs, for instance, keep their existing left-aligned block position; only the heading above them moves to center. (Hero's `<h1>` is not affected — its left-anchored layout alongside the avatar is a distinct composition, not a "section heading" in this sense.) After implementing, visually verify the centered heading doesn't read as disconnected from a left-anchored content block beneath it — if it looks off, flag it rather than silently also recentering the content, since only the heading change was requested.

### Skills section layout
- Revised from a multi-column category grid to a **single centered column, stacked vertically** ("staircase" layout) — each category block (icon + label, then its badges) sits centered, one after another down the page, rather than side-by-side columns.
- Constrain the column to a readable max-width (don't stretch it to the full container width) so it doesn't look lost/adrift on wide desktop viewports — center that narrower column within the section.
- Category icon + label centered as a unit; badges below each category also centered (wrapped, not left-aligned).
